...
def render_recapture_panel(
    pills: list[PillViewModel],
    on_recapture: Callable[[str, Image.Image], None],
    on_manual_override: Callable[[str, str], None],
    errors: dict[str, str] | None = None,
) -> None:
    """Render minimal per-pill retake controls for medications not yet identified."""
    pending = [pill for pill in pills if pill.status != "accepted" and not pill.is_manual_override]
    if not pending:
        return

    errors = errors or {}
    st.markdown("### Retake only the medications below")
    st.caption(
        "Only medications that are not yet identified are listed. "
        "A retake keeps the same medication ID."
    )
    ...
    for pill in pending:
        with st.container(border=True):
            ...
            with detail_col:
                st.markdown(f"**{pill.instance_id}**")
                ...
                with st.expander("Retake or manually confirm", expanded=False):
