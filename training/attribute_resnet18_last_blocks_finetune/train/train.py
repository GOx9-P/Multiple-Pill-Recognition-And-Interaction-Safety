...
def train():
    ...
    # 1. LOAD CHECKPOINT TỪ GIAI ĐOẠN HEAD TUNE (QUAN TRỌNG)
    pretrained_path = "experiments/attribute_resnet18_head_tune/checkpoints/attr_head_v1_best.pt"
    if os.path.exists(pretrained_path):
        print(f"[Info] Đang load trọng số tốt nhất từ: {pretrained_path}")
        model.load_state_dict(torch.load(pretrained_path, map_location=device))
    else:
        print(f"[Warning] Không tìm thấy file {pretrained_path}! Model đang khởi tạo từ ImageNet.")

    # 2. CẤU HÌNH CẬP NHẬT TRỌNG SỐ CHO LAYER4 VÀ HEADS
    for name, param in model.named_parameters():
        # Mở khóa nếu tên chứa layer4 hoặc các từ khóa liên quan đến head/fc
        if 'layer4' in name or 'head' in name or 'fc' in name:
            param.requires_grad = True
        else:
            param.requires_grad = False

    # 3. OPTIMIZER CHỈ TỐI ƯU CÁC THAM SỐ CÓ requires_grad=True
    trainable_params = [p for p in model.parameters() if p.requires_grad]
    print(f"[Info] Số lượng tensor tham số được train: {len(trainable_params)}")

    if len(trainable_params) == 0:
        raise ValueError("Không có tham số nào được bật requires_grad! Hãy kiểm tra lại tên các layer trong vòng lặp.")
